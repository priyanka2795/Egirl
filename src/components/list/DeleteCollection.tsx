...
const DeleteCollection = ({closeDeleteModal}: DeleteCollectionModalProp) => {
  return (
    <Modal
    open={true}
  ...
  <div>  
     <div className='flex justify-between w-full gap-2 p-6 border-b border-white/[0.08]'>
        <div className='flex flex-col items-start gap-1 text-white text-lg font-bold w-[286px]'>Delete collection
            ...
            <div className="flex top-[7px] items-start gap-4 w-[24px] h-[24px] object-contain cursor-pointer" onClick={() => closeDeleteModal(false)}>
                <CloseIcon/>
            </div>           
    </div>
        ...
        <div className='w-full px-6 pt-5 pb-6'>
            <div className='flex gap-3 w-ful'>
                <button className='rounded-[14px] w-1/2 px-[20px] py-[11px] border border-white/[32] text-base font-bold' onClick={() => closeDeleteModal(false)}>Cancel</button>
                <button className='rounded-[14px] w-1/2 px-[20px] py-[11px] bg-[#FF5336] text-base text-white font-bold' onClick={() => closeDeleteModal(false)}>Delete</button>
            </div>
        </div>    
