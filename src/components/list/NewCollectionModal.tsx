...
const NewCollectionModal = ({closeModalItem, closeDropdown} : CollectionModalProp) => {
  return (
    <Modal
    open={true}
  ...
  <div>  
    <div className='flex justify-between w-full gap-2 p-6 border-b border-white/[0.08]'>
        ...
        <div className='flex top-[7px] items-start gap-4 w-[24px] h-[24px] object-contain' onClick={() => {closeModalItem(false), closeDropdown('')}}>
            <CloseIcon/>
        </div>
    </div>
        ...
        <div className='w-full px-6 pt-3 pb-6'>
            ...
            <div className='flex gap-[12px] w-full mt-[24px]'>
                <button className='rounded-[14px] w-1/2 px-[20px] py-[13px] border border-white/[0.32] text-[16px] font-bold' onClick={() => {closeModalItem(false), closeDropdown('')}}>Cancel</button>
                <button className='rounded-[14px] w-1/2 px-[20px] py-[13px] bg-[#5848BC] text-[16px] text-[#FFFFFF] font-bold' onClick={() => {closeModalItem(false), closeDropdown('')}}>Create</button>
            </div>
        </div>    
