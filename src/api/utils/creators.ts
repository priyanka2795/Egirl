...
export async function getCreator(creator_id: number) {
  ...
  const characters = getCharactersByCreatorId(creator_id);

  return { data, total_subscriptions, characters };
}

export async function getCharactersByCreatorId(creator_id: number) {
  let { data, error, status } = await supabaseClient
    .from('characters')
    .select(
...
export async function getCreatorSubscriptionsByCreatorId(creator_id: number) {
  let { data, error, status } = await supabaseClient
